...
\textbf{In this case, the actions are continuous, thus it is reasonable to use Gaussian distribution for the actions, centered around$\phi$$(s, a)^T$$\theta$, which is  a $\sim$ N($\phi$$(s, a)^T$$\theta$, $\sigma^2$)} \\
...
\textbf{a $\sim$ N($\phi$$(s, a)^T$$\theta$, $\sigma^2$)}  \\
...
\noindent \textbf{Now we would like to analyze how $G_t$ and $\theta$ are updated.} 
...
\textbf{The input vector has a dimension of 4. (x, x', $\theta$, $\theta$')} \\
...
The episode will not finish if the cart's position within (-2.4, 2.4) and the pole's position within (-24$\pi$/360, 24$\pi$/360). After the training process, the cart and the pole have a tendency to be balanced, that means x = 0, and $\theta$  = 0 \\
Observing our output, the weights in 2 different columns suggest moving in 2 opposite directions.\\ 

Considering the position of the cart,  the vector in the first row of the weights matrix [ 0.36846304  0.79150385] suggests that in the following action the cart be positioned on the side with a higher weighting for this vector (corresponding to the right column). We claim our output to be reasonable. If you look at the vector [ 0.04124827 -0.13071053] with respect to the velocity of cart, which is in the second row of the weight matrix. The higher weight appears to be in the left column, this indicates the cart to be moving towards the direction corresponding to the left column. \textbf{The first two vectors together suggest the cart has a tendency towards the middle point.}  \\

Considering the position of the pole, the vector in the third row of the weights matrix [ 0.06586583 -0.25080118] suggests that in the following action the pole be positioned on the side with a higher weighting for this vector (corresponding to the left column). Again, we claim our output to be reasonable. If you look at the vector [-1.09515846  1.89147353] with respect to the angular velocity of pole, which is in the last row of the weight matrix. The higher weight appears to be in the right column, this indicates the pole to be moving towards the direction corresponding to the right column. \textbf{The last two vectors together suggest the pole has a tendency to stay perpendicular to the horizontal axis.} \\

Overall, the cart and pole system has a tendency to be balanced, which is as expected after the training process. \vspace{5mm}
